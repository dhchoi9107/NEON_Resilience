...
chk("MODIS vs Hill q1 r", rm, 0.60, tol=0.006); chk("  p", pm, 0.001, tol=0.0007)
chk("PML vs tower r",   st.pearsonr(tv2.pml, tv2.tower_gpp)[0], 0.86, tol=0.006)
chk("MODIS vs tower r", st.pearsonr(tv2.modis, tv2.tower_gpp)[0], 0.85, tol=0.007)
...
o4 = pd.read_csv(os.path.join(RES,"o4_interactions_consistent.csv"))
chk_int("screen terms", len(o4), 168)
chk_int("FDR-surviving", int((o4.q<0.05).sum()), 7)
fam = o4[o4.q<0.05].family.value_counts().to_dict()
chk_int("  disturbance", fam.get("disturbance",0), 4)
chk_int("  land use",    fam.get("land use",0), 3)
ss_ = pd.read_csv(os.path.join(RES,"simple_slopes.csv"))
gA = ss_[(ss_.rp=="Hill q1")&(ss_.rs=="modis_gpp")&(ss_.ctx=="stand_age_gami")].sort_values("ctx_z")
gB = ss_[(ss_.rp=="Hill q1")&(ss_.rs=="VCI_mean")&(ss_.ctx=="severity")].sort_values("ctx_z")
for v,w in zip(gA.slope,[0.17,0.27,0.38]): chk("GPPxage slope", v, w, tol=0.006)
