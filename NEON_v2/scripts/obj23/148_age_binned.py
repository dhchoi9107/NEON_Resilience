...
# figure: 4 key structural trends by 20yr bin (boxplot)
...
for ax,(t,l) in zip(axes,KEY):
    data=[df[df.age_bin==b][t].dropna().values for b in order]
    ax.boxplot(data,showfliers=False)
    ax.set_xticklabels([f"{int(b.left)}-{int(b.right)}" for b in order],rotation=45,ha='right',fontsize=8)
    mid=[(b.left+b.right)/2 for b in order]; med=[np.median(x) if len(x) else np.nan for x in data]
    ax.plot(range(1,len(order)+1),med,'r-o',lw=2,ms=4)
    ax.axhline(0,color='gray',ls=':'); ax.set_title(f"{l}_trend ~ 임령구간"); ax.set_xlabel("임령 구간(년)"); ax.set_ylabel(f"{t}"); ax.grid(alpha=.2)
fig.suptitle("N11. 구조 복잡화 변화율 ~ 20년 임령구간 (GAMI 권고 binning) — 젊음=축적↑, 노령=갭↑ 유지",fontsize=13)
fig.tight_layout(); fig.savefig(f"{F}/N11_age_binned_trends.png",dpi=120,bbox_inches='tight'); plt.close()
print("\nsaved N11_age_binned_trends.png")
