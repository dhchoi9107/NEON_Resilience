...
Outputs: results/stand_age_{models,moderation,trendage}.csv, figures/N01_stand_age_26.png
...
for src,col in [("gami","stand_age_gami"),("30m","stand_age_30m")]:
    fp=os.path.join(D,f"plot_stand_age_{src}_26.csv")
...
for src,acol in AGES.items():
    ...
    md=pd.DataFrame(rows); md.to_csv(os.path.join(R,f"stand_age_models_{src}_26.csv"),index=False)
    ...
    mo=pd.DataFrame(rows); mo.to_csv(os.path.join(R,f"stand_age_moderation_{src}_26.csv"),index=False)
    ...
    mt=pd.DataFrame(rows); mt.to_csv(os.path.join(R,f"stand_age_trendage_{src}_26.csv"),index=False)
...
fig.tight_layout(); fig.savefig(os.path.join(F,"N01_stand_age_26.png"),dpi=120,bbox_inches='tight'); plt.close()
print("\nsaved N01_stand_age_26.png ; DONE")
