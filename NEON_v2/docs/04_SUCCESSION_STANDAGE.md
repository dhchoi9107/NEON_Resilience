...
### 다양성 오버레이 (`N07`, 탐색)
임령 × VCI_trend 산점도에 **점 크기 = 종다양성 4지수**(색=site): 알파(Hill q1/q2) 큰 점은 중간연령·높은 VCI_trend 동부 군집(MLBS/HARV ~80–110년), **nestedness 큰 점은 젊은 ABBY**(젊은/재생 임분=nested 종빈약 부분집합, 생태적 타당). turnover는 중간 군집 분산. → 다양성이 임령×복잡화 공간에서 구조적으로 분포.
