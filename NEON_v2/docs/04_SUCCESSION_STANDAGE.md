...
- **그러나 domain 통제 시 소멸**(혼합모델 2차 p=0.99). = **생물지리 교란**: 중간연령 ≈ 동부 낙엽수림(고 수종다양성 ~85–110년, MLBS/HARV/SCBI), 양끝 ≈ 저다양성(젊은 관리림 ABBY/JERC + 노령 서부 침엽수 WREF/TEAK). **순수 임령혹형 아님**(중간연령에 우연히 고다양성 동부림이 위치).
