...
- ★★ **혹형은 생산성 현상이 아니라 DHI(녹색도) 아티팩트** — GPP 삼각검증으로 반전(`L05·L06`, `dhi_gpp_triangulation.csv`):
  - DHI 혹형은 forest type 통제엔 생존(L04, β=−0.287)하나, **독립 생산성 GPP로는 혹형 소멸·반전**: MODIS GPP β=+0.22, PML GPP β=+0.17 (**둘 다 단조증가=혹형 아님**).
  - **두 GPP는 서로 r=0.85 일치**하는데 **DHI는 GPP와 0.46~0.49**(중간)뿐 → **DHI가 아웃라이어.** 혹형 하강부(고DHI=저다양)는 **밀집 상록 캐노피의 NDVI 포화**(ABBY: NDVI 최고·GPP 중간·다양성 최저) 산물.
  - **★ 실측 NEON 타워 GPP(gold standard, 16사이트, `L07`)로 최종확정**: 타워와 상관 **DHI 0.35(약) vs MODIS 0.67·PML 0.69**, 다양성~타워GPP 2차 β=+0.14(혹형 아님). = 위성2종+실측타워 3중 검증 모두 혹형 부정.
  - → **진짜 관계 = 생산성↑ → 다양성↑ 단조(종-에너지 가설)**. "생산성–다양성 혹형"은 **위성 녹색도 아티팩트**로 재프레이밍. **임령혹형(생물지리)·DHI혹형(녹색도) 둘 다 겉보기 혹형이 검증서 무너짐** = 프로젝트 통일 방법론 교훈.
- 교란강도·구조·분광다양성은 선형/단조(중간교란가설 미지지).
- 맵핑 — 스크립트 `60·61`(DHI)·`120~122`(비선형)·`147`(forest type)·`149·150·151`(MODIS·PML·타워 GPP 검증) / 그림 `L03`·`L04`·**L05·L06·L07**(GPP 검증) / 결과 `dhi_gpp_validation`·`dhi_gpp_triangulation`·`tower_gpp_validation`
